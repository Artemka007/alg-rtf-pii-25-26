...
std::vector<int> calc_lcp(const std::vector<int> &val, const std::vector<int> &sa) {
    ...
    // LCP между n суффиксами = n-1 значений
    std::vector<int> lcp(n - 1);
    
    // Строим массив rank (обратный к sa)
    for (int i = 0; i < n; i++) {
        rank[sa[i]] = i;
    }
    ...
    for (int i = 0; i < n; i++) {
        if (rank[i] > 0) {
            int j = sa[rank[i] - 1];
            // Находим LCP между суффиксами, начинающимися в i и j
...
// Основная функция для подсчета различных подстрок
long long count_distinct_substrings(const std::string &str) {
    if (str.empty()) return 0;
    
    int n = str.size();
    
    // Преобразуем строку в вектор чисел (от 1 до 26)
    std::vector<int> s(n);
    for (int i = 0; i < n; i++) {
        s[i] = str[i] - 'a' + 1;
    }
    ...
    // Проверяем размер суффиксного массива
    if (sa.size() != n) {
    ...
    // Подсчитываем общее количество подстрок
    long long total_substrings = (long long)n * (n + 1) / 2;
    
    // Вычитаем повторяющиеся подстроки (сумма LCP)
